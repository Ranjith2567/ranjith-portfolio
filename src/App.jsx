...
function App() {
  ...
  const [messages, setMessages] = useState([
    { sender: 'bot', text: "SYSTEM ONLINE: Welcome! I am Ranjith's personal AI assistant. Please select an option below to explore his tech stack, projects, and engineering experience." }
  ]);
  ...
  return (
    <div className={`w-full min-h-screen font-sans antialiased overflow-x-hidden selection:bg-orange-500 selection:text-white relative transition-colors duration-500 ${isDarkMode ? 'bg-[#0a0604] text-slate-100' : 'bg-[#fafafa] text-slate-900'
      }`}>

      {/* AMBIENT BACKGROUND GLOW */}
      <div className={`absolute top-0 left-0 w-full h-[100vh] pointer-events-none z-0 transition-opacity duration-500 ${isDarkMode ? 'opacity-100' : 'opacity-0'
        }`}>
      ...
      <motion.header
        ...
        className={`w-full fixed top-0 backdrop-blur-md z-40 transition-colors duration-300 ${isDarkMode ? 'bg-[#0a0604]/80 border-b border-white/5' : 'bg-white/90 border-b border-slate-200'
          }`}
      >
        <div className="max-w-[1400px] mx-auto h-20 px-6 md:px-12 flex justify-between items-center">
          ...
          <div className={`hidden lg:flex space-x-10 text-xs font-bold uppercase tracking-widest ${isDarkMode ? 'text-slate-300' : 'text-slate-600'
            }`}>
          ...
          <div className="flex items-center gap-4">
            <button
              ...
              className={`p-2 rounded-lg border text-sm transition-all active:scale-95 cursor-pointer ${isDarkMode ? 'border-white/10 bg-white/5 text-slate-300 hover:bg-white/10' : 'border-slate-300 bg-slate-100 text-amber-600 hover:bg-slate-200'
                }`}
            >
              {isDarkMode ? '☀️' : '🌙'}
            </button>
            ...
            <button
              ...
              className={`lg:hidden p-2 rounded-xl border transition-all active:scale-90 cursor-pointer ${isDarkMode ? 'border-white/10 bg-white/5 text-slate-300' : 'border-slate-300 bg-slate-50 text-slate-800'
                }`}
            >
        ...
        <AnimatePresence>
          {isMobileMenuOpen && (
            <motion.div
              ...
              className={`w-full lg:hidden border-b overflow-hidden font-mono ${isDarkMode ? 'bg-[#0a0604] border-white/5' : 'bg-slate-50 border-slate-200'
                }`}
            >
